...
drop function if exists public.get_public_promotions_page(
  date,
  text,
  integer,
  integer
...
create function public.get_public_promotions_page(
...
as $$
  ...
  order by
    case
      when filtered.promotion_price is null then 1
      else 0
    end asc,
